...
exports.handleSOSAlert = (0, firestore_1.onDocumentCreated)({ document: `${FS_SOS_EVENTS}/{sosId}`, ..._sosHandlerOptions }, async (event) => {
    ...
    allUsersSnap.forEach((doc) => {
        if (doc.id === senderUid)
            return; // don't notify the originator
    ...
    if (tokens.length === 0) {
        console.log('handleSOSAlert: no other users have an fcmToken, skipping');
        return;
    }
    console.log(`handleSOSAlert: sending FCM to ${tokens.length} device(s) across all roles`);
